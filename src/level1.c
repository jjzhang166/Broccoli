...
const static uint32_t SX1278_Channels[] = { SX1278_Channel0, SX1278_Channel1,
		SX1278_Channel2, SX1278_Channel3, SX1278_Channel4, SX1278_Channel5,
		SX1278_Channel6, SX1278_Channel7 };
...
uint8_t Radio_RXBusy(void) {
	return SX1278_RXBUSY;
}

uint8_t Radio_TXBusy(void) {
	return SX1278_TXBUSY;
}
	...
	if ((RF_EX0_STATUS & 0x40) == 0x40) {
		...
		if ((CRC_Value & 0x04) == 0x04) {
			...
			Radio_RXData(buf, length,
					-137 + SX1278ReadBuffer(REG_LR_RSSIVALUE));
			SX1278_RXBUSY = 0;
		} else {
			SX1278WriteBuffer( REG_LR_IRQFLAGS, 0xff);
